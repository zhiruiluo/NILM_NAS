...
class TrainingManager:
    def __init__(self, args: MyProgramArgs):
        ...
        self.persistence = PersistenceFactory(db_name=self.args.systemOption.db_name, db_dir=self.args.systemOption.db_dir).get_persistence()
...
def trainable(config: dict, debug=False):
    ...
    try:
        ...
        if args.nasOption.enable:
            results.nas_params = json.dumps({'params': count_parameters(model, trainable_only=False), "trainable_params": count_parameters(model, trainable_only=True)})
    ...
    return {'val_acc': results.val_metrics.acc, 'val_f1macro': results.val_metrics.f1macro}
