...
class MultilabelDataset(Dataset):
    def __init__(self, dc, seq_to_point: bool = True) -> None:
        self.dc = dc
        ...
        labels = []
        for k in keys:
            labels.append(torch.tensor(self.dc[k], dtype=torch.float32))
            
        self.target = torch.stack(labels, axis=2)
...
def data_augmentation(train):
    ...
    df_train_resampled, df_y_resampled = sampler.fit_resample(df_train, df_train_last['powerlabel'])
    # df_y_resampled.hist(figsize=(5,5))
    # plt.savefig('results/resampled.png')
    return df_train_resampled.to_dict('list')
    

class REDD(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> DataLoader:
        return self._to_dataloader(
            self.train_set, True, self.args.modelBaseConfig.batch_size, num_workers=0, drop_last=False
        )
