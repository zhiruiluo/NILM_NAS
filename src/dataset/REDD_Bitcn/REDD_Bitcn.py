...
class Seq2pointDataset(Dataset):
    def __init__(self, np_data: np.ndarray, win_size: int, stride: int) -> None:
        ...
        win_view = sliding_window_view(np.arange(self.np_data.shape[0]), window_shape=win_size)
    ...
    def __getitem__(self, index):
        ...
        return {'input': torch.tensor(self.np_data[win_indices,0],dtype=torch.float32), 
...
class REDD_Bitcn(pl.LightningDataModule):
    ...
    def visualize(self):
        ...
        with disk_buffer(func=load_redd_bitcn, keys=str(self.config.appliance), folder=folder) as bf_fn:
    ...
    def prepare_data(self) -> None:
        ...
        self.train_set = Seq2pointDataset(train,self.config.win_size, self.config.stride)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return self._to_dataloader(
            self.train_set, True, self.args.modelBaseConfig.batch_size, num_workers=2, drop_last=False, sampler=sampler
        )
