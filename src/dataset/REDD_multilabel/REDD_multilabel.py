...
class Seq2PointMultilabelDataset(Dataset):
    def __init__(self, df_data: pd.DataFrame, selected_channels: List[str],
                 combine_mains: bool = False, win_size: int = 300, stride: int = 1,
                 transform=None) -> None:
    ...
    def __getitem__(self, index):
        ...
        sample = {'input': self.input[win_indices, 0],
...
class MultilabelDataset(Dataset):
    def __init__(self, dc, selected_channels: List[str], seq_to_point: bool = True, combine_mains: bool = False, transform=None) -> None:
        self.dc = dc
        self.selected_channels = selected_channels
        ...
        self.input = np.stack(
            (self.dc.get('mains_1', None), self.dc.get('mains_2', None)), dtype=np.float32)
...
def apply_transform(train: dict[str, list], val: dict[str, list], test: dict[str, list], t):
...
class REDD_multilabel(pl.LightningDataModule):
    ...
    def visualize(self):
        ...
        with disk_buffer(func=dataset_by_house, keys=str(self.config.house_no), folder=folder) as bf_dataset_by_house:
    ...
    def prepare_data(self):
        ...
        selected_channels = [
            appliances[f'house_{self.config.house_no}'][app] for app in self.config.appliances]
        with disk_buffer(func=get_dataset, keys=str(self.config.house_no)+'redd_ml_s2p', folder=folder) as bf_get_dataset:
            train, val, test = bf_get_dataset(house_no=self.config.house_no, data_root=self.data_root, channels=[
                                              s_ch for s_chs in selected_channels for s_ch in s_chs])
        ...
        self.train_set = Seq2PointMultilabelDataset(
            train, selected_channels, win_size=self.config.win_size, stride=self.config.stride, transform=transform)
        self.val_set = Seq2PointMultilabelDataset(
            val, selected_channels, win_size=self.config.win_size, stride=self.config.stride, transform=transform)
        self.test_set = Seq2PointMultilabelDataset(
            test, selected_channels, win_size=self.config.win_size, stride=self.config.stride, transform=transform)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return self._to_dataloader(
            self.train_set, True, self.args.modelBaseConfig.batch_size, num_workers=0, drop_last=False, sampler=sampler
        )

    def val_dataloader(self) -> DataLoader:
        return self._to_dataloader(
            self.val_set, False, self.args.modelBaseConfig.val_batch_size, num_workers=1, drop_last=False
        )
