...
def get_dataset(house_no: int, data_root: str, channels: list, 
                sample_seconds: int = 6, fillna_limit=1):
    ...
    df_house = df_house.resample(str(sample_seconds) + 'S').mean().fillna(method='backfill', limit=fillna_limit)
