...
class MinMax(object):
    ...
    def __call__(self, sample: Any) -> Any:
        for i, k in enumerate(self.scalers.keys()):
            ...
            sample['input'][i] = scaler.transform(sample['input'][i].reshape(1,-1)).reshape(-1)
        return sample

def power(x, np_arange):
    return np.sum(np_arange*x)

class ApplyPowerLabel(object):
    def __init__(self) -> None:
        self.power_func_partial = None
    
    def __call__(self, sample) -> Any:
        if self.power_func_partial is None:
            self.power_func_partial = functools.partial(power,np_arange=np.arange(len(sample['target']))**2)
        sample['powerlabel'] = np.apply_along_axis(self.power_func_partial, 0, sample['target'])
        return sample
