...
def get_n_var_with_channels(n_phases: int, n_ops: int) -> int:
    return CHANNEL_BITS*n_phases + n_phases*((n_ops * (n_ops-1)//2)+1 + n_ops*3)

def get_n_var_repeat_with_channels(n_phases: int, n_ops: int) -> int:
    return CHANNEL_BITS*n_phases + (n_ops * (n_ops-1)//2)+1 + n_ops*3

def decode_channels(bit_string: str, n_phases: int):
    channel_bits = bit_string[0: n_phases]
    ...
    rest_bit_string = bit_string[n_phases:]
    return channels, rest_bit_string
...
class NASProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out_channels, bit_string = decode_channels(''.join(['1' if i else '0' for i in x]), config['args'].modelConfig.n_phases)
        ...
        if not self.debug:
            ...
            result['flops'] = self._n_evaluated
...
def get_tsnet_problem_with_channels(args: MyProgramArgs, pop_size: int, monitor: str, mode: str, elementwise_runner=None, checkpoint_flag: bool= True, debug=False):
    '''
    args: MyProgramArgs
    pop_size: int
    monitor: str
...
def test_nasproblem_binary():
    ...
    problem, algorithm, checkpoint  = get_tsnet_problem_with_channels(args, pop_size=20, monitor='val_f1macro', mode='max', checkpoint_flag=False, debug=False)
