...
from torchmetrics.classification import (BinaryF1Score, MultilabelAccuracy,
                                         MultilabelF1Score)
...
def get_metrics(names: List[str] | str, nclass: int, multilabel: bool = False) -> Dict[str, ]:
    """
    >>> import torch
...
class Metrics_Helper():
    ...
    @staticmethod
    def from_results(results: Dict[str, Any], 
                     start_time: datetime.datetime, 
                     training_time: datetime.timedelta, 
                     macs: int = None, 
                     flops: int = None, 
                     params: Dict = {}) -> ExpResults:
        res = {}
        logger.info(results)
