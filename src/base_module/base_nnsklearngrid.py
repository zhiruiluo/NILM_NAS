...
from src.config_options.modelbase_configs import NNSklearnGridBaseConfig
...
class NNSklearnGridBaseModule:
    ...
    def fit_nn(self, dataset):
        ...
        results = trainer_fac.training_flow(trainer, self.get_nnmodel(), dataset, no_test=True)
        return results
    ...
    def share_nnsklearn_epoch(self, phase, dl):
        ...
        with disk_buffer(self.dl_to_feature, keys=f'{self.args.datasetConfig.question_no}_{phase}', folder=f'.temp/{self.get_features_folder()}') as dl_to_feature_:
...
def training_flow_nnsklearngrid(args, model: NNSklearnGridBaseModule, dataset) -> ExpResults:
    ...
    for p in tqdm(ParameterGrid(model.get_grid())):
        ...
        persistence = PersistenceFactory(db_name=args.systemOption.db_name, db_dir=args.systemOption.db_dir).get_persistence()
