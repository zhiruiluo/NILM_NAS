import logging

import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.nn.utils import spectral_norm

from src.model.loss.asymmetric_loss import AsymmetricLossOptimized

logger = logging.getLogger(__name__)

class MultilabelSharedLinearASL(nn.Module):
    def __init__(self, in_dim, nclass, dropout=0.5) -> None:
        super().__init__()
        self.nclass = nclass
        self.head = nn.Sequential(
            nn.Dropout(dropout),
            spectral_norm(nn.Linear(in_dim, nclass)),
        )
        
        self.loss_fn = AsymmetricLossOptimized()
    
    def loss(self, predictions, batch):
        pred = predictions["pred"]
        target = batch["target"].type(torch.long)
        # target = F.one_hot(target).type(torch.float32)
        return self.loss_fn(pred, target)
    
    def forward(self, batch):
        x = batch['feature']
        out = self.head(x)
        preds = {}
        preds['pred'] = out
        preds['output'] = torch.sigmoid(out)
        preds['loss'] = self.loss(preds, batch)
        return preds
    
def test_MultilabelSharedLinearASL():
    m = MultilabelSharedLinearASL(64, 4)
    batch = {
        "feature": torch.randn(2, 64),
        "target": torch.empty((2, 4), dtype=torch.long).random_(2),
    }
    out = m(batch)
    print(out)