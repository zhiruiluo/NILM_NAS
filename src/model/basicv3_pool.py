...
class BasicV3_Pool(LightningBaseModule):
    def __init__(self, args: MyProgramArgs) -> None:
        ...
        self.conv1 = nn.Conv2d(1, 1, kernel_size=11, stride=4, padding=get_padding_one_more_or_same(11))
        self.fcn = FCN(
            config.in_channels,
            chan_list=chan_list,
            ker_list=ker_list,
...
def test_basicV3():
    ...
    opt = OptionManager()
    args = opt.replace_params({'modelConfig': 'BasicV2','modelBaseConfig.label_mode': "multiclass"})
    ...
    flops, macs, params = get_model_profile(model, args=[batch],as_string=False, print_profile=False)  #flops: 3756395, macs: 1864650, params: 4537
