...
class SlurmConfig(Serializable):
    ...
    def __post_init__(self):
        ...
        self.job_name = '{}_{}'.format(
            self.exp_name, time.strftime('%m%d_%H%M', time.localtime()),
        )
...
class SlurmDeploy:
    ...
    def _modify_func_decorator(
        self, module_source: str, func_source: str, modify_func: str,
    ):
    ...
    def _generate_file(self):
        ...
        modified_func_source = self._extract_func_def(func_source)
        modifyied_source = self._modify_func_decorator(
            module_source, func_source, modified_func_source,
        )
    ...
    def _launch(self):
        ...
        process = subprocess.Popen(
            ['sbatch', self.script_file], stdout=subprocess.PIPE, text=True,
        )
        ...
        print(
            'Job submitted! Script file is at: <{}>. Log file is at: <{}>'.format(
                self.script_file,
                self.args.slurm_path.joinpath(f'{self.args.job_name}.log'),
            ),
        )
        # sys.exit(0)
