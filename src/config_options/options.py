...
def replace_subgroups(obj: DataclassT, selections: Optional[Dict] = None) -> DataclassT:
    """
...
def replace(obj: DataclassT, changes_dict: Optional[Dict] = None, **changes) -> DataclassT:
...
def loads_json(s) -> MyProgramArgs:
    ...
    conf.modelBaseConfig = ModelbaseGroups().get_modelbase_by_model(conf.modelConfig.__class__).from_dict(modelBaseConfig)
...
class OptionManager():
    ...
    def _replace_subgroups(self, changes: Dict, ori_subgroup):
        subgroups = {}
        subgroups['datasetConfig'] = changes.get('datasetConfig') if changes.get('datasetConfig') is not None else ori_subgroup['datasetConfig']
        subgroups['modelConfig'] = changes.get('modelConfig') if changes.get('modelConfig') is not None else ori_subgroup['modelConfig']
        subgroups['modelBaseConfig'] = changes.get('modelBaseConfig') if changes.get('modelBaseConfig') is not None else ori_subgroup['modelBaseConfig']
    ...
    def _parse_default(self, argv, default):
        return simple_parsing.parse_known_args(MyProgramArgs, args=argv, default=default)
    ...
    def _on_pre_build(self):
        ...
        if jobid is not None:
            ...
            args.systemOption.job_name = 'time_{}'.format(time.strftime("%m%d_%H%M", time.localtime()))
    ...
    def _make_consist_for_subgroups(self, args, subgroups):
        ...
        dataset_config_cls = select_field(MyProgramArgs, 'datasetConfig').metadata['subgroups'][subgroups['datasetConfig']]
        if not isinstance(args.datasetConfig, dataset_config_cls):
            args.datasetConfig = dataset_config_cls()
            
        model_config_cls = select_field(MyProgramArgs, 'modelConfig').metadata['subgroups'][subgroups['modelConfig']]
        if not isinstance(args.modelConfig, model_config_cls):
            args.modelConfig = model_config_cls()

        model_base_config_cls = ModelbaseGroups().get_modelbase_by_model(args.modelConfig.__class__)
        if not isinstance(args.modelBaseConfig, model_base_config_cls):
            args.modelBaseConfig = model_base_config_cls()

    def _replace_by_dict_known(self, src: MyProgramArgs, changes: dict) -> MyProgramArgs:
