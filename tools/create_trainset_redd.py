...
def main():
    ...
    for h in params_appliance[appliance_name]['houses']:
        chn_num = str(params_appliance[appliance_name]['channels'][params_appliance[appliance_name]['houses'].index(h)])
        ...
        threshold = 5
        df_align[appliance_name] = df_align[appliance_name].apply(lambda x: 1 if x > threshold else 0)
        # df_align[appliance_name] = (df_align[appliance_name] - mean) / std

        if h == params_appliance[appliance_name]['test_build']:
            # Test CSV
            df_align.to_csv(Path(args.save_path).joinpath(f'{appliance_name}_test_.csv').as_posix(), mode='a', index=False, header=False)
            print("    Size of test set is {:.4f} M rows.".format(len(df_align) / 10 ** 6))
            continue


        train = pd.concat([train, df_align], axis=0, ignore_index=True)
    ...
    val.to_csv(Path(args.save_path).joinpath(f'{appliance_name}_validation_.csv').as_posix(), mode='a', index=False, header=False)

        # Training CSV
    train.to_csv(Path(args.save_path).joinpath(f'{appliance_name}_train_.csv').as_posix(), mode='a', index=False, header=False)
