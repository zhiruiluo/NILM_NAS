...
def loop(args: MyProgramArgs):
    ...
    space = {
        "datasetConfig": {
            # "appliances": tune.grid_search([["washer_dryer","dishwasher"],["dishwasher","microwave"],["microwave","washer_dryer"],["refrigerator","microwave"]]),
            "combine_mains": tune.grid_search([True])
        },
        "modelConfig": {
            "head_type": tune.grid_search(['Focal']),
            "in_chan": tune.sample_from(lambda spec: 1 if spec.config["datasetConfig"]["combine_mains"] else 2),
        },
        # "modelConfig.in_chan": tune.sample_from(lambda spec: 1 if spec.config["datasetConfig"]["combine_mains"] else 2),
    }
