...
def parse_best_tsnet_json():
    with open('./run/ukdale/multilabel/best_pf_f1macro_424_lstm.json', mode='r') as fp:
...
def loop(args: MyProgramArgs):
    ...
    space = {
        ...
        "datasetConfig": {
            ...
            "house_no": tune.sample_from(lambda spec: spec.config.model['house_no']),
        },
        "modelConfig":{
            "n_phases": 3,
            "n_ops": 5,
            ...
            "head_type": "ASL",
            "atten": True,
            "lstm_out_features": tune.sample_from(lambda spec: spec.config.model['lstm_out_features']),
            "lstm_hidden_features": tune.sample_from(lambda spec: spec.config.model['lstm_hidden_features']),
        }
    }
