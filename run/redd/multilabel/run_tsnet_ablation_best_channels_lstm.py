...
def parse_best_tsnet_json():
    with open('./run/redd/multilabel/best_f1macro_424_lstm.json', mode='r') as fp:
...
def loop(args: MyProgramArgs):
    ...
    space = {
        ...
        "modelConfig":{
            ...
            "head_type": "ASL",
            "atten": True,
            "lstm_out_features": tune.sample_from(functools.partial(get_best_config, list_tsnets=list_tsnets,key='lstm_out_features')),
            "lstm_hidden_features": tune.sample_from(functools.partial(get_best_config, list_tsnets=list_tsnets,key='lstm_hidden_features')),
...
@slurm_launch(
    exp_name="TSNET_Ablation",
    num_nodes=1,
    num_gpus=2,
    partition="epscor",
    log_dir='logging/REDD_lstm',
